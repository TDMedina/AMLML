...
class ConnectedLayers(nn.Module):
    def __init__(self, n_genes, shrinkage_factor=10, minimum_penultimate_size=10,
                 final_size=1, zero_params=False, kaiming_weights=False, leakyrelu=0,
...
class ShallowConnectedLayers(nn.Module):
    def __init__(self, input_size, dropout=0.2,
                 ...
                 cutoff=300, leakyrelu=0):
        super().__init__()

        hidden1 = self.compute_hidden_size(input_size)
        layers = [nn.Linear(input_size, hidden1), nn.LeakyReLU(leakyrelu), nn.Dropout(dropout)]

        if input_size > cutoff:
            hidden2 = max(hidden1 // 2, 32)
            layers += [nn.Linear(hidden1, hidden2), nn.LeakyReLU(leakyrelu), nn.Dropout(dropout)]
            final_in = hidden2
        else:
            final_in = hidden1
