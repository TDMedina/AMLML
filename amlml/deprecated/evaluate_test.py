...
def test_run(train: NetworkDataset, test: NetworkDataset,
    ...
    filter_params = [filter_events, minimum_duration, filter_minimum_censorship, remove_age_over, restrict_tech]
    classify_params = [use_rmst, rmst_max_time, rmst_tukey_factor, classification_threshold, filter_ambiguous]
    datasets = []
    for dataset in [train, test]:
        dataset = prefilter_dataset(dataset, *filter_params)
        if classify or hazard_classify:
            dataset = preclassify_dataset(dataset, *classify_params)
